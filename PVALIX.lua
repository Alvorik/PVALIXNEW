...
local Paragraph = Credits:CreateParagraph({Title = "Project™", Content = "Project Valix / Project™ 4.2 | Made by notvixinity™#5160"})
...
elseif game.PlaceId == 6516141723 then
   local DOORS = Window:CreateTab("DOORS") -- Title, Image
   local BreakInSection = DOORS:CreateSection("DOORS Stuff")

   local Paragraph = DOORS:CreateParagraph({Title = "Project Valix", Content = "Project Valix 4.2 | DOORS "})

   -- Values | 

   getgenv().Fullbrightness = true
   
   -- Functions |

   function fullbrightness()
      if getgenv().Fullbrightness == true then
         
         Rayfield:Notify({
            Title = "Project Valix",
            Content = "Turning On FULL BRIGHTNESS | Light Warning",
            Duration = 6.5,
            Image = 0,
            Actions = { -- Notification Buttons
               Ignore = {
   ...
   local speedslider = DOORS:CreateSlider({
      Name = "WalkSpeed",
      Range = {16, 22},
      Increment = 10,
      Suffix = "WalkSpeed",
